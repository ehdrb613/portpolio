...
//1. 업로드 파일 존재여부 확인
	if(!isset($_FILES['upload'])) {
		echo "<script>alert('업로드 파일 존재하지 않음');</script>";
		exit;
	}//if
...
//2. 업로드 오류여부 확인
 	if ($_FILES['upload']['error'] > 0) {
		switch ($_FILES['upload']['error']) {
			case 1:
			    echo "<script>alert('php.ini 파일의 upload_max_filesize 설정값을 초과함(업로드 최대용량 초과)');</script>";
				exit;
			case 2:
				echo "<script>alert('50Mb를 초과했습니다.(업로드 최대용량 초과);</script>";
				exit;
			case 3:
				echo "<script>alert('파일 일부만 업로드 됨');</script>";
				exit;
			case 4:
				echo "<script>alert('업로드된 파일이 없음');</script>";
				exit;
			case 6:
				echo "<script>alert('사용가능한 임시폴더가 없음');</script>";
				exit;
			case 7:
				echo "<script>alert('디스크에 저장할수 없음');</script>";
				exit;
			case 8:
				echo "<script>alert('파일 업로드가 중지됨');</script>";
				exit;
			default:
				echo "<script>alert('시스템 오류가 발생');</script>";
				exit;
		} // switch
	}//if
...
//3. 업로드 제한용량 체크(서버측에서 50M로 제한)
	if($_FILES['upload']['size'] > 52428800) {
		echo "<script>alert('업로드 최대용량 초과;</script>";
		exit;
	}//if
...
//5. MIME를 통해 이미지파일만 허용(2차 확인)
	...
	if(!in_array($_FILES['upload']['type'], $ableImage)) {
		echo "<script>alert('지정된 이미지만 허용됩니다.');</script>";
		exit;
	}//if
...
//6. DB에 저장할 이미지 정보 가져오기(width,height 값 활용)
	...
	if(mysqli_connect_errno()) {
		echo "<script>alert('DB연결오류.;</script>";
		exit;
	}//if
...
//8. db에 업로드 파일 및 새로 생성된 파일정보등을 저장
	$query = sprintf("INSERT INTO grida_imege
		(upload_filename,db_filename,file_path,file_size,file_type,upload_date,ip,width,height) 
		VALUES ('%s','%s','%s',%d,'%s','%s','%s',%d,%d)",
		$upload_filename, $fileName, $filePath, $file_size, $file_type, $upload_date, $ip,$img_size[0],$img_size[1]);
...
//10. 성공시 db저장 내용을 적용(커밋)
			$db->commit();
			 echo "<script>alert('다음페이지로 넘어갑니다.'; location.href='../htp_test/h_q.html';</script>";
		}else{
