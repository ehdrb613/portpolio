...
mysql_query("set names utf8");
    ...
    $sql = "SELECT id FROM grida_member ";
...
mysql_query("set names utf8");
    ...
    $sql = "SELECT id FROM grida_h_t WHERE ok = '미등록' ";
    $result = mysql_query($sql);
    $row = mysql_fetch_assoc($result);
...
<body>
    ...
    <div id="title">보관함</div>
     </header><input id="qu" class="bu2" type="button" value="답변" />
	<table>
...
?>
		<tr>
				<td>
				<!--이미지 넣을곳-->
				<div> <?php echo $id?>님의 H검사 </div>
				<?php

 		
        $sql2 = "SELECT * FROM grida_h_t WHERE id = '$id'";
...
?>
    ...
    <img class="img_size" src="../t_img/h_t_img/<?php echo $fn_v; ?>">
		...
		<?php
		$sql= "SELECT * FROM grida_h_t WHERE id = '$id' && ok='미등록'";
...
?>
		...
		<tr>
				...
				<div> T검사 </div>
				<?php
				$sql2 = "SELECT * FROM grida_t_t WHERE id = '$id'";
        $result2 = mysql_query($sql2);
      	$row = mysql_fetch_assoc($result2);
            $fn_v=$row["fn"];
            ?>
		...
		<?php
			$sql= "SELECT * FROM grida_t_t WHERE id = '$id' && ok='미등록'";
...
?>
		<tr>
				...
				<div> P1검사 </div>
				<?php
				$sql2 = "SELECT * FROM grida_p_t1 WHERE id = '$id'";
        	$result2 = mysql_query($sql2); 
      		$row = mysql_fetch_assoc($result2);
            $fn_v=$row["fn"];
            ?>
		...
		<?php
		$sql= "SELECT * FROM grida_p_t1 WHERE id = '$id' && ok='미등록'";
...
?>
		...
		<tr>
				...
				<div> P2검사 </div>
				<?php
				$sql2 = "SELECT * FROM grida_p_t2 WHERE id = '$id'";
        $result2 = mysql_query($sql2);
      	$row = mysql_fetch_assoc($result2);
            $fn_v=$row["fn"];
            ?>
		...
		<?php
		$sql= "SELECT * FROM grida_p_t2 WHERE id = '$id' && ok='미등록'";
